...
public class Game extends Application {
	...
	public void start(Stage primaryStage) throws FileNotFoundException {
		...
		FileReader fileReader=new FileReader("CSE1242_spring2022_project_level" + currentLevel.getId() + ".txt"); /*For reading the files based on current level*/
		Circle gameBall = FileReader.getBall(); /*Gets ball from filereader*/
		ArrayList<Tile> all_tiles=fileReader.start_reading(); /*Gets an array list full of tiles from filereader*/
		setBall(gameBall,CheckPath.findStarterIndex(all_tiles)); /*Sets the ball location*/
		
		BorderPane borderPane=new BorderPane();/*Border pane for storing gridpane buttons and counter*/
		
		setGridPane(gridPane, all_tiles); /*Adds tiles to the gridpane accordingly*/
		...
		gridPane.setOnMousePressed(e->{
			...
			if(isFinished[0] == false) {
				...
				currentTileObject[0] = all_tiles.get(columnIndex + rowIndex * 4); /*Finds the array equilavent of the mouse location*/
		...
		gridPane.setOnMouseReleased(e->{
			...
			if(isFinished[0] == false) {
				...
				if(otherTile instanceof Free && currentTile instanceof Movable) {
				...
				if(((currentColumn == otherColumn) && (rowDifference == -1 || rowDifference == 1)) ||
					...
					/*If the path is not null,basically if the path is correct or leading to the end*/
						if(currentPathTileArray != null) { 
							...
							circlePathTransition.setPath(CircleAnimation.returnCirclePath(currentPathTileArray,all_tiles)); /*Finds the path from the path array*/
